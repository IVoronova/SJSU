...
public class FamilyTree 
{
	...
	private void addLine(String line) throws TreeException
	{
		...
		if (colonIndex < 0)
			throw new TreeException("Incorrect Format: Colon not found");//?? throw a TreeException with a useful message
		...
		else
		{
			...
			if(parentNode == null)
				throw new TreeException("Incorrect Format: incorrect order of family members");//??? If the parent node wasn't found, there must have been something wrong in the 
			
				//data file. Throw an exception.
		}
		
		for(String child: childrenArray) {
			parentNode.addChild(new TreeNode(child));// Add child nodes to parentNode.
		}
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null)
			throw new TreeException("No family memeber with name " + name1);//Throw a TreeException with a useful message
		TreeNode node2 = root.getNodeWithName(name2);		// node whose name is name2
		if (node2 == null)
			throw new TreeException("No family member with name " + name2); //??? Throw TreeException with a useful message
