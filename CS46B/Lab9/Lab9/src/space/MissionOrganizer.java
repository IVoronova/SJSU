...
public class MissionOrganizer implements Iterable<Mission>
{
	...
	private ArrayList<Mission> cheapList;
	private TreeSet<Mission> expensiveList;
	...
	{
		return cheapList.contains(m) || expensiveList.contains(m);
	}
	...
	public void add(Mission mission)
	{
		if(!contains(mission)) {
			if(mission.getCost() < ONE_TRILLION)
				cheapList.add(mission);
			else
				expensiveList.add(mission);
	...
	public Iterator<Mission> iterator()
	{
		...
		missionList.addAll(cheapList);
		missionList.addAll(expensiveList);
		return missionList.iterator();
	...
	private final static Mission[]		TEST_MISSIONS =
	{
        ...
        // New
        new Mission("Far Far away planet", 3E13f),
        new Mission("The Citadel of Ricks", 69E9f),
        new Mission("Blips and Chitz", 5E12f),
        new Mission("My basement", 100f)
